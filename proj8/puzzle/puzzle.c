...
int main (int argc, char *argv[]) {
   const int NUM_THREADS = 4;     //constant num threads 4
   int numProcesses = 0, processID = 0, value = 0; //init
   ...
   omp_set_num_threads( NUM_THREADS ); //4 threads
   #pragma omp parallel reduction(+:value) 
   {
      value = omp_get_thread_num(); //my guess is it will equal 0+1+2+3 with three p's it should print this three times??
   }
