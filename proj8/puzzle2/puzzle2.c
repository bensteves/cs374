...
int main (int argc, char *argv[]) {
   ...
   omp_set_num_threads( NUM_THREADS ); //num threads is 4

	/* Leaving master uncommented - reduction is only done by one thread, which is master, so nothing
	* but 1 is added
	*
	* Commenting it out - adds "1" 4 times since there are four threads. Otherwise essentially, only the master is 
