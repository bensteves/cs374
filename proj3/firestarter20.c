/* firestarter.c 
 * by David Joiner
 * Edited by Ben Steves - CS374 9-18-21
...
int main(int argc, char ** argv) {
	...
	//save different copies for different forest sizes
    int forest_size=20;
    double * prob_spread;
    double prob_min=0.0;
    double prob_max=1.0;
    ...
    local_count = (double *) malloc (n_probs*sizeof(double));
    global_count = (double *) malloc (n_probs*sizeof(double));
    ...
    for (i_prob = 0; i_prob < n_probs; i_prob++) {
        ...
        for (i_trial=id; i_trial < n_trials; i_trial+=numProcesses) {
            ...
            local_count[i_prob]+=burn_until_out(forest_size,forest,prob_spread[i_prob],
	...
	MPI_Reduce(local_count, global_count, n_probs, MPI_DOUBLE, 
				MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(percent_burned, global_percent_burned, n_probs, MPI_DOUBLE, 
				MPI_SUM, 0, MPI_COMM_WORLD);
	...
	if (id == 0) {
		...
		for (i_prob = 0; i_prob < n_probs; i_prob++) {
			...
			global_count[i_prob]/=n_trials;
			printf("%lf , %lf, %i\n",prob_spread[i_prob],
					global_percent_burned[i_prob],
					(int)round(global_count[i_prob]));
		}
