...
/**
 * Vector square: C = pow(A, 2).
 *
 * This sample is a very basic sample that implements element by element
 * vector square root. It is the same as the sample illustrating Chapter 2
 ...
 *
 * Computes the vector square of A into C 
 * The 2 vectors have the same number of elements numElements.
 */
__global__
...
int main(int argc, char** argv)
{
    double startTime = 0.0, totalTime = 0.0,
    	   ABcopyTime = 0.0, totalABcopyTime = 0.0,
    	   squareTime = 0.0, totalSquareTime = 0.0,
    	   CcopyTime = 0.0, totalCcopyTime = 0.0,
    	   seqTime = 0.0, totalSeqTime = 0.0;
    ...
    printf("[Vector squaring of %lu elements]\n", numElements);
   ...
   // printf("CUDA kernel launch with %d blocks of %d threads\n", blocksPerGrid, threadsPerBlock);
    ...
    totalSquareTime = omp_get_wtime() - squareTime;
...
//    printf("Normal time: %lf\n", stopTime-startTime); 
   ...
   // free(h_B);
    ...
    printf("Cuda runtime: %f \nSequential runtime: %f\n", totalTime, totalSeqTime);
    printf("AB Copy: %f \nSquare: %f \nC copy: %f \n", totalABcopyTime, totalSquareTime, totalCcopyTime);
