 ...
 * vector multiplication. It is the same as the sample illustrating Chapter 2
 ...
 *
 * Computes the vector product of A and B into C. 
...
int main(int argc, char** argv)
{
    double startTime = 0.0, totalTime = 0.0,
    	   ...
    	   productTime = 0.0, totalProductTime = 0.0,
   ...
   // printf("Copy input data from the host memory to the CUDA device\n");
    ...
    productTime = omp_get_wtime();
    // 3. Launch the Vector Multiply CUDA Kernel
   ...
   // printf("CUDA kernel launch with %d blocks of %d threads\n", blocksPerGrid, threadsPerBlock);
    ...
    totalProductTime = omp_get_wtime() - productTime;
...
//    printf("Normal time: %lf\n", stopTime-startTime); 
    ...
    printf("AB Copy: %f \nProduct: %f \nC copy: %f \n", totalABcopyTime, totalProductTime, totalCcopyTime);
